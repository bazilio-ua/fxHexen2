...
//
	...
	//lerp timing, but only if interval is not 0.1 (0.05 for H2) (which client assumes)
		...
		if (!ent->free && ent->v.nextthink > sv.time && (ent->v.movetype == MOVETYPE_STEP || ent->v.movetype == MOVETYPE_WALK || ent->v.frame != ent->oldframe))
		{
			...
			if (j >= 0 && j < 256 && j != 12 && j != 13 && j != 25 && j != 26) //25 and 26 are close enough to 0.1 to not send (in accordance 12 and 13 close to 0.05 for H2, that we also don't want to send)
