...
void CL_SendMove (usercmd_t *cmd)
{
	...
	//
		...
		if (!cls.demoplayback && (cls.netcon->mod == MOD_PROQ) && (cl.protocol <= PROTOCOL_RAVEN_112)) // precise aim for ProQuake
		{
			for (i=0 ; i<3 ; i++)
				MSG_WritePreciseAngle (&buf, cl.viewangles[i]);
		}
		else if (cl.protocol == PROTOCOL_FITZQ || cl.protocol == PROTOCOL_MARKV || cl.protocol == PROTOCOL_RMQ) //johnfitz -- 16-bit angles for PROTOCOL_FITZQUAKE
		{
			for (i=0 ; i<3 ; i++)
