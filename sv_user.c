...
// read current angles	
	...
	else if (sv.protocol == PROTOCOL_FITZQ || sv.protocol == PROTOCOL_MARKV || sv.protocol == PROTOCOL_RMQ) //johnfitz -- 16-bit angles for PROTOCOL_FITZQUAKE
	{
		for (i=0 ; i<3 ; i++)
...
//				Sys_Printf ("SV_ReadClientMessage: clc_nop\n");
			...
			case clc_stringcmd:	
				...
				if (strncasecmp(s, "status", 6) == 0)
					...
					ret = 1;
				else if (strncasecmp(s, "playerclass", 11) == 0)
					ret = 1;
